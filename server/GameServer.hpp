...
struct GameServer:public std::enable_shared_from_this<GameServer> {
  ...
  using OnMessageCallback = std::function<void(std::shared_ptr<Client>, std::unique_ptr<MessageBase>)>;
  using OnConnectCallback = std::function<void(uint64_t, std::shared_ptr<Client>)>;
 ...
 public:
  class Client: public std::enable_shared_from_this<Client> {
    ...
    uint64_t id;
    Client(std::shared_ptr<GameServer> s, uint64_t id) : serverInst(s), id(id){};
    friend std::shared_ptr<Client> GameServer::createClient(uint64_t);

   public:
    Client() = default;
    void send(std::string &&message) {
      serverInst->sendMessage(shared_from_this(), std::forward<std::string>(message));
    }
